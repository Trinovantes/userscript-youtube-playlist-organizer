...
const { numChecked, toggleAll, moveAllToPlaylist, removeAll, addAllToQueue, addAllToWatchLater } = usePlaylistPageCheckboxes()
const disableBulkAction = computed(() => numChecked.value === 0)

const enableAll = ref(false)
watch(enableAll, async () => {
    await toggleAll(enableAll.value)
})
...
<template>
    <Teleport to="#primary > ytd-section-list-renderer > #header-container">
        ...
        >
            <button
                @click="moveAllToPlaylist(targetPlaylist)"
